...
def loadata(filename):
    ...
    with open(filename) as f:
        # use to pass the first line with the headers
...
#                feel float,metar text, PRIMARY KEY((lon,lat),year,month,day,hour,minute));"""
...
# def writecdr_by_datetime(csvfilename):
#     from cassandra.cluster import Cluster
#     session=Cluster(["localhost"])
#     db=session.connect("groupe_td2_1718")

#     # Table creation
#     sql_create="""CREATE TABLE asos_datetime(station text,year int, month int, day int, hour int, minute int, lon float,lat float,
#                tmpf float,dwpf float,relh float,drct float,sknt float,p01i float,alti float,mslp float,vsby float,gust float,
#                skyc1 text ,skyc2 text,skyc3 text,skyc4 text,
#                skyl1 float,skyl2 float,skyl3 float,skyl4 float,
#                wxcodes text, ice_accretion_1hr float,ice_accretion_3hr float,ice_accretion_6hr float,
#                peak_wind_gust float, peak_wind_drct float, peak_wind_time float,
#                feel float,metar text, PRIMARY KEY( (year,month,day,hour), minute, station ));"""
#     db.execute(sql_create)
#     print('\n TABLE "asos_datetime" CREATED')

#     inserted = 0
#     for data in loadata(csvfilename):
#             sql=f"""INSERT INTO asos_datetime(station,year,month,day,hour,minute,lon,lat,
#             tmpf,dwpf,relh,drct,sknt,p01i,alti,mslp,vsby,gust,skyc1,skyc2,skyc3,skyc4,
#             skyl1,skyl2,skyl3,skyl4,wxcodes,ice_accretion_1hr,ice_accretion_3hr,ice_accretion_6hr,
#             peak_wind_gust,peak_wind_drct,peak_wind_time,feel,metar) 
#                VALUES ('{data["station"]}',{data["valid"][0]},{data["valid"][1]},{data["valid"][2]},{data["valid"][3]},{data["valid"][4]},
#               {data["lon"]},{data["lat"]},{data["tmpf"]},
#               {data["dwpf"]},{data["relh"]},{data["drct"]},
#               {data["sknt"]},{data["p01i"]},
#               {data["alti"]},{data["mslp"]},{data["vsby"]},
#               {data["gust"]},'{data["skyc1"]}',
#               '{data["skyc2"]}','{data["skyc3"]}','{data["skyc4"]}',
#               {data["skyl1"]},{data["skyl2"]},
#               {data["skyl3"]},{data["skyl4"]},'{data["wxcodes"]}',
#               {data["ice_accretion_1hr"]},{data["ice_accretion_3hr"]},
#               {data["ice_accretion_6hr"]},{data["peak_wind_gust"]},
#               {data["peak_wind_drct"]},{data["peak_wind_time"]},
#               {data["feel"]},'{data["metar"]}');"""
#             db.execute(sql)
#             inserted += 1
#             print(f" \n INSERTED : {inserted}")
#     db.shutdown()
...
def writecdr_by_day(csvfilename):
    from cassandra.cluster import Cluster
    import cassandra
    session=Cluster(["localhost"])
    db=session.connect("groupe_td2_1718")
    db.execute("drop table asos_day;")
    # Table creation
    sql_create="""CREATE TABLE asos_day(station text, timestamp_day timestamp, hour int, minute int, lon float,lat float,
               tmpf float,dwpf float,relh float,drct float,sknt float,p01i float,alti float,mslp float,vsby float,gust float,
               skyc1 text ,skyc2 text,skyc3 text,skyc4 text,
               skyl1 float,skyl2 float,skyl3 float,skyl4 float,
               wxcodes text, ice_accretion_1hr float,ice_accretion_3hr float,ice_accretion_6hr float,
               peak_wind_gust float, peak_wind_drct float, peak_wind_time float,
               feel float,metar text, PRIMARY KEY( (timestamp_day), hour, minute, station ));"""
    db.execute(sql_create)
    print('\n TABLE "asos_day" CREATED')

    inserted = 0
    for data in loadata(csvfilename):
        timestamp_day = f'{data["valid"][0]}-{data["valid"][1]}-{data["valid"][2]} 00:00'
        sql=f"""INSERT INTO asos_day(station, timestamp_day,hour,minute,lon,lat,
        tmpf,dwpf,relh,drct,sknt,p01i,alti,mslp,vsby,gust,skyc1,skyc2,skyc3,skyc4,
        skyl1,skyl2,skyl3,skyl4,wxcodes,ice_accretion_1hr,ice_accretion_3hr,ice_accretion_6hr,
        peak_wind_gust,peak_wind_drct,peak_wind_time,feel,metar) 
            VALUES ('{data["station"]}',
            '{timestamp_day}',
            {data["valid"][3]},{data["valid"][4]},
            {data["lon"]},{data["lat"]},{data["tmpf"]},
            {data["dwpf"]},{data["relh"]},{data["drct"]},
            {data["sknt"]},{data["p01i"]},
            {data["alti"]},{data["mslp"]},{data["vsby"]},
